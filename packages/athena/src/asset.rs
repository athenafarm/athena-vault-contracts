...
use cosmwasm_std::{
    to_binary, Addr, Api, BankMsg, Binary, CanonicalAddr, Coin, CosmosMsg, Decimal, MessageInfo,
...
impl Asset {
    ...
    pub fn into_msg_with_data(
        ...
        match &self.info {
            ...
            AssetInfo::NativeToken { .. } => Ok(CosmosMsg::Wasm(WasmMsg::Execute {
                contract_addr: recipient.to_string(),
                funds: vec![self.deduct_tax(querier)?],
                msg: data,
            })),
        }
    }
...
impl Into<TerraswapAsset> for Asset {
    fn into(self) -> TerraswapAsset {
        TerraswapAsset {
            amount: self.amount,
            info: self.info.into(),
        }
...
impl Into<Asset> for TerraswapAsset {
    fn into(self) -> Asset {
        Asset {
            amount: self.amount,
            info: self.info.into(),
...
impl Into<TerraswapAssetInfo> for AssetInfo {
    fn into(self) -> TerraswapAssetInfo {
        match self {
            AssetInfo::Token { contract_addr } => TerraswapAssetInfo::Token { contract_addr },
            AssetInfo::NativeToken { denom } => TerraswapAssetInfo::NativeToken { denom },
        }
    }
}

impl Into<AssetInfo> for TerraswapAssetInfo {
    fn into(self) -> AssetInfo {
        match self {
            TerraswapAssetInfo::Token { contract_addr } => AssetInfo::Token { contract_addr },
    ...
    pub fn query_balance(
        ...
        match self {
            AssetInfoRaw::Token { contract_addr } => {
                query_token_balance(querier, api.addr_humanize(contract_addr)?, addr)
            }
            AssetInfoRaw::NativeToken { denom } => query_balance(querier, addr, denom.to_string()),
        }
    }

    pub fn query_sent_fund(&self, message_info: &MessageInfo) -> StdResult<Uint128> {
        if let AssetInfoRaw::NativeToken { denom } = &self {
            let amount: Uint128 = message_info
                .funds
                .iter()
    ...
    pub fn equal(&self, asset: &AssetInfoRaw) -> bool {
        match self {
            AssetInfoRaw::Token { contract_addr } => {
                let self_contract_addr = contract_addr;
                match asset {
                    AssetInfoRaw::Token { contract_addr } => self_contract_addr == contract_addr,
